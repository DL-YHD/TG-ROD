...
def train(config: dict, resume: str, device_name: str):
    ...
    if config['name'] == 'TGRAPMNet':
        # Assuming that the predicted results pred and uncertainty (with shapes [B, T, H, W, C]) have been obtained
        ...
        clip_model, clip_preprocess = clip.load(config['model']['clip_model']['model_path'], device=device) 
    ...
    loss_history = LossHistory(exp_dir, config['name']) # Loss Training Curve
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if config['name'] == 'TGRAPMNet' and use_extra_information:
                ...
                # uncertainty_map = torch.exp(0.5 * outputs['class_log_var']) # Equivalent to opening the root number

                # # Calculate weight: The lower the uncertainty, the higher the weight (taking the average uncertainty of all categories)
                # uncertainty_map = 1.0 / (uncertainty_map + 1e-8)  # Low uncertainty → high weight
                # uncertainty_map = uncertainty_map / uncertainty_map.max()  # Normalized weight
                # uncertainty_pred = uncertainty_map * outputs['output']

                # uncertainty_pred_loss = loss_fn(uncertainty_pred, confmap)
                # Filter: Only retain predictions with uncertainty below the threshold, and mark the rest as' unknown '
                # reliable_mask = uncertainty_map < threshold  # Generate masks [B, 1, T, H, W], True indicates reliability
                # filtered_pred = outputs['output'] * reliable_mask + (-1) * (~reliable_mask)  # Unreliable area marked as -1
