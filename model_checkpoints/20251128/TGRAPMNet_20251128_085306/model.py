...
# from FDConv.FDConv_detection.mmdet_custom import FDConv
from tools.lgpm import Mixer3D
...
class UncertaintyEstimationNetwork(nn.Module):
    """
    不确定度估计网络，处理形状为(B, T, H, W, D)的输入特征
    
    输出:
    - mean: 预测均值 (B, T, H, W, D)
    - log_var: 预测对数方差 (B, T, H, W, D)，用于表示不确定度
    """
    def __init__(self, in_channels, hidden_channels=64, kernel_size=3):
        super(UncertaintyEstimationNetwork, self).__init__()
        padding = kernel_size // 2
        
        # 时空特征提取 backbone
        self.feature_extractor = nn.Sequential(
            # 3D卷积，捕获时空特征
            nn.Conv3d(in_channels, hidden_channels, 
            ...
            # 第二个3D卷积层
            nn.Conv3d(hidden_channels, hidden_channels * 2,
                      kernel_size=(kernel_size, kernel_size, 3),
                      padding=(padding, padding, 1)),
            ...
            # 降维卷积
            nn.Conv3d(hidden_channels * 2, hidden_channels,
                      kernel_size=(1, 1, 1)),
            nn.BatchNorm3d(hidden_channels),
            nn.ReLU(inplace=True)
        )
        ...
        # 均值预测头
        self.mean_head = nn.Conv3d(hidden_channels, in_channels,
                                  kernel_size=(1, 1, 1),
                                  padding=0)
        
        # 方差预测头 (预测对数方差更稳定)
        self.log_var_head = nn.Conv3d(hidden_channels, in_channels,
                                     kernel_size=(1, 1, 1),
                                     padding=0)
    ...
    def forward(self, x, min_value=5, max_value=5):
        # x 形状: (B, C, T, H, W)
        
        # 提取特征
        features = self.feature_extractor(x)  # (B, hidden_channels, T, H, W)
        
        # 预测均值和对数方差
        mean = self.mean_head(features)  # (B, 1, T, H, W)
        log_var = self.log_var_head(features)  # (B, 1, T, H, W)
        
        # 限制方差的范围，避免数值不稳定
        log_var = torch.clamp(log_var, min=min_value, max=max_value)
...
class mRadNet(nn.Module):
    def __init__(
        ...
        self.cls_uncertainty_flag = model_cfg['cls_uncertainty']
        self.mix3d_flag = model_cfg['mix3d']
        ...
        self.class_uncertainty = UncertaintyEstimationNetwork(in_channels=model_cfg['window_size']) if self.cls_uncertainty_flag else False

        self.mixer = Mixer3D(dim = 2*num_chirps) if self.mix3d_flag else False
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        B, T, chirp, R, A, C = x.shape # 4, 16, 4, 128, 128, 2
        

        x_p = x.permute(0, 1, 5, 2, 3, 4).contiguous()  # B T C chirp H W
        x_in = torch.zeros((B, T, 2 * chirp, R, A), device=x_p.device)

        for i in range(T):
            # B T C chirp H W -> B T 8 H W
            x_in[:, i] = self.stem(x_p[:, i]).squeeze(2)

        if self.mix3d_flag:
            x_in_mix = self.mixer(x_in) # New added [4, 16, 8, 128, 128] 
            x_in_p = x_in_mix.permute(0, 1, 3, 4, 2) # B T 8 H W -> B T H W 8 [4, 16, 8, 128, 128] -> [4, 16, 128, 128, 8]
        else:
            x_in_p = x_in.permute(0, 1, 3, 4, 2)  # B T 8 H W -> B T H W 8

        features = self.encoder(x_in_p) # [[4, 8, 64, 64, 48], [4, 8, 32, 32, 96], [4, 8, 16, 16, 192], [4, 8, 8, 8, 384]]

        output = self.decoder(features) # [4, 16, 128, 128, 3]

        # output = torch.sigmoid(output) # New added

        if self.cls_uncertainty_flag:
            class_mean, class_log_var = self.class_uncertainty(output) # New added
            return {'output': output, 'class_mean':class_mean, 'class_log_var':class_log_var}
        else:
