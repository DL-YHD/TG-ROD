...
def visualize_test_img(fig_name, img_path, input_radar, confmap_pred, confmap_gt, res_final, config, viz=False,
                       img_save_split=False):
    ...
    if img_save_split == False:
        ...
        plt.imshow(img_data.astype(np.uint8))
        plt.axis('off')
        plt.savefig(fig_name.replace('.jpg', '_rgb.jpg'), bbox_inches='tight', pad_inches = -0.1)
        plt.clf()

        plt.imshow(input_radar, origin='lower', aspect='auto')
        plt.axis('off')
        plt.savefig(fig_name.replace('.jpg', '_radar.jpg'), bbox_inches='tight', pad_inches = -0.1)
        plt.clf()
        ...
        for d in range(max_dets):
            ...
            plt.text(col_id + 5, row_id, text, color='white', fontsize=10)
        plt.axis('off')
        plt.savefig(fig_name.replace('.jpg', '_pred.jpg'), bbox_inches='tight', pad_inches = -0.1)
        plt.clf()

        confmap_gt = np.transpose(confmap_gt, (1, 2, 0))
        plt.imshow(confmap_gt, vmin=0, vmax=1, origin='lower', aspect='auto')
        plt.axis('off')
        plt.savefig(fig_name.replace('.jpg', '_gt.jpg'), bbox_inches='tight', pad_inches = -0.1)
        plt.clf()


def visualize_test_img_wo_gt(fig_name, img_path, input_radar, confmap_pred, res_final, config, viz=False,
                             sybl=False):
    max_dets, _ = res_final.shape
    classes = config['dataset']['class_names']
